...
impl DwClient {
    ...
    ///
    /// Corresponds to `POST /ai/v1/files`.
    pub async fn upload_file(&self, path: &Path, purpose: &str) -> Result<FileResponse, DwError> {
        ...
        let request = self.post(ApiSurface::Ai, "/ai/v1/files")?.multipart(form);
        self.send(request).await
    }

    /// List uploaded files.
    ///
    /// Corresponds to `GET /ai/v1/files`.
    pub async fn list_files(&self, params: &ListFilesParams) -> Result<FileListResponse, DwError> {
        let request = self.get(ApiSurface::Ai, "/ai/v1/files")?.query(params);
        self.send(request).await
    }

    /// Get a specific file's metadata.
    ///
    /// Corresponds to `GET /ai/v1/files/{file_id}`.
    pub async fn get_file(&self, file_id: &str) -> Result<FileResponse, DwError> {
        let request = self.get(ApiSurface::Ai, &format!("/ai/v1/files/{}", file_id))?;
        self.send(request).await
    }

    /// Delete a file.
    ///
    /// Corresponds to `DELETE /ai/v1/files/{file_id}`.
    pub async fn delete_file(&self, file_id: &str) -> Result<(), DwError> {
        let request = self.delete(ApiSurface::Ai, &format!("/ai/v1/files/{}", file_id))?;
        self.send_empty(request).await
    }

    /// Get a file's content (raw bytes).
    ///
    /// Corresponds to `GET /ai/v1/files/{file_id}/content`.
    pub async fn get_file_content(&self, file_id: &str) -> Result<bytes::Bytes, DwError> {
        let request = self.get(ApiSurface::Ai, &format!("/ai/v1/files/{}/content", file_id))?;
        self.send_bytes(request).await
    }

    /// Get cost estimate for processing a file.
    ///
    /// Corresponds to `GET /ai/v1/files/{file_id}/cost-estimate`.
    pub async fn get_file_cost_estimate(
        ...
        let mut request = self.get(
            ApiSurface::Ai,
            &format!("/ai/v1/files/{}/cost-estimate", file_id),
        )?;
